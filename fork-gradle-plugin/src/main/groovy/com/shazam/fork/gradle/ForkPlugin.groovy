...
class ForkPlugin implements Plugin<Project> {
    ...
    private static ForkRunTask createTask(final TestVariant variant, final Project project) {
        ...
        forkTask.configure {
            ...
            failureRetryRegex = config.failureRetryRegex
            testClassAnnotation = config.testClassAnnotation
            enableLeakCanaryDump = config.enableLeakCanaryDump
            testClassAnnotation = config.testClassAnnotation
            denyPermissionsAnnotation = config.denyPermissionsAnnotation
